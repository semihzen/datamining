...
def evaluate_model(data_file, method_name, results):
    ...
    # SVM model eğitimi
    model = SVC(kernel='linear')  # SVM with linear kernel
...
def compare_methods_svm(result_folders, output_file):
    results = []
    ...
    print(f"SVM Sonuçları kaydedildi: {output_file}")

# Unigram ve Bigram klasörleri
result_folders = ['unigram_results', 'bigram_results']

# Çıktı dosyası
output_excel_svm = 'svm_results_comparison.xlsx'

# Yöntemleri karşılaştır ve Excel'e yaz
compare_methods_svm(result_folders, output_excel_svm)
